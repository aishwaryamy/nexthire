from fastapi import APIRouter, Depends, HTTPException, BackgroundTasks
from sqlalchemy.orm import Session
from database import get_db
from models import Job
from agents.pattern_analyzer import analyze_patterns
from agents.job_discovery import fetch_jobs_from_serper, score_jobs, build_search_query
from agents.email_notify import send_job_alert_email
from config import settings
from pydantic import BaseModel
from typing import Optional
import json

router = APIRouter(prefix="/api/discover", tags=["discover"])

class DiscoverRequest(BaseModel):
    resume: Optional[str] = ""
    custom_query: Optional[str] = None
    send_email: bool = False
    alert_email: Optional[str] = None

@router.get("/patterns")
async def get_patterns(db: Session = Depends(get_db)):
    jobs = db.query(Job).all()
    jobs_data = [{
        "company": j.company, "role": j.role, "status": j.status,
        "remote": j.remote, "sponsorship": j.sponsorship,
        "location": j.location, "salary_range": j.salary_range,
        "parsed_keywords": j.parsed_keywords,
    } for j in jobs]
    return await analyze_patterns(jobs_data)

@router.post("/jobs")
async def discover_jobs(payload: DiscoverRequest,
                        background_tasks: BackgroundTasks,
                        db: Session = Depends(get_db)):
    jobs = db.query(Job).all()
    jobs_data = [{"company": j.company, "role": j.role, "status": j.status,
                  "remote": j.remote, "sponsorship": j.sponsorship,
                  "location": j.location, "parsed_keywords": j.parsed_keywords}
                 for j in jobs]

    pattern_result = await analyze_patterns(jobs_data)
    success_profile = pattern_result.get("success_profile", {})

    query = payload.custom_query or (
        await build_search_query(success_profile) if success_profile
        else "machine learning engineer LLM RAG FastAPI 2026 job"
    )

    raw_jobs = await fetch_jobs_from_serper(query, num=10)
    scored = await score_jobs(raw_jobs, success_profile, payload.resume or "")

    # Send email alert if requested and high-scoring jobs found
    high_scoring = [j for j in scored if j.get("score", 0) >= 75]
    email_sent = False
    if payload.send_email and high_scoring:
        to_email = payload.alert_email or settings.ALERT_EMAIL
        if to_email:
            background_tasks.add_task(
                send_job_alert_email, to_email, high_scoring, query
            )
            email_sent = True

    return {
        "query_used": query,
        "success_profile": success_profile,
        "jobs": scored,
        "total": len(scored),
        "high_scoring_count": len(high_scoring),
        "email_sent": email_sent,
    }

@router.get("/stats")
async def get_discovery_stats(db: Session = Depends(get_db)):
    jobs = db.query(Job).all()
    total = len(jobs)
    successes = sum(1 for j in jobs if j.status in ["phone_screen","interview","offer"])
    rejections = sum(1 for j in jobs if j.status == "rejected")
    pending = sum(1 for j in jobs if j.status in ["saved","applied"])
    rate = round(successes / total * 100, 1) if total > 0 else 0
    return {
        "total": total, "successes": successes, "rejections": rejections,
        "pending": pending, "call_rate": rate,
        "has_enough_data": (successes + rejections) >= 3,
        "email_configured": bool(settings.SMTP_EMAIL and settings.SMTP_PASSWORD),
        "alert_email": settings.ALERT_EMAIL or ""
    }
